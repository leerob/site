...
Most software is *deterministic*. Given some input, if you run the program again, you will get the same output. A developer has explicitly written code to handle each case.

AI models are not this way — they are *probabilistic*. Developers don’t have to explicitly program the instructions.

Machine learning teaches software to recognize patterns from data. Given some input, if you run the program again, you might *not* get the same output. AI models like GPT (from OpenAI), Claude (from Anthropic), and Gemini (from Google) are “trained” on a large chunk of internet documents. These models learn patterns during training.

Then, there’s an API or chat interface where you can talk to the model. Based on some input, it can predict and generate sentences, images, or audio as output.

## Neural Networks

AI models are built on [neural networks](https://www.youtube.com/watch?v=aircAruvnKk&list=PLZHQObOWTQDNU6R1_67000Dx_ZCJB-3pi) — which are like a digital version of a brain.

<LLM />

These networks consist of multiple layers of interconnected _neurons_ that process information:
...
2.  Many **hidden layers** that create an understanding of patterns in the system. Neurons inside the layer apply *weight* to the input data and pass the result through an “activation function”. This function outputs a value, often between 0 and 1, representing the neuron's level of activation.
...
These additional layers enable the network to learn complex patterns. In language models, for instance, deep networks can understand virtually every written language. You can ask a question in English, and get a response back in Japanese.
...
To handle the complexity of human language—including rare words and misspellings—modern models use subword tokenization techniques like [**Byte Pair Encoding (BPE)**](https://www.perplexity.ai/search/what-is-byte-pair-encoding-bpe-MV_w_T17RWG6sZUqRl2pIA). BPE starts with individual characters and iteratively merges the most frequent pairs of symbols to form new tokens.

For instance, the word "unbelievable" might be tokenized into `["un", "believ", "able"]`. This allows the model to understand and generate words it has not explicitly seen during training. Newer models have also developed specialized tokenizers for processing math and code, since both have unique syntax and structure.

## Training

Large Language Models (LLMs) are trained on vast amounts of internet data.
...
2.  **Loss Function**: After obtaining the output, we need to measure how good or bad the prediction is. This is done using a **loss function**, which quantifies the difference between the network's prediction and the actual target output.
...
The core innovation of transformers is the use of **attention mechanisms**, specifically **self-attention**. Unlike previous models that read sequences word by word, transformers can consider the entire input sequence at once. This allows the model to focus on different parts of the input when generating each part of the output, much like how a human understands language by considering the context of a sentence or paragraph.
...
After pretraining, the model may need to be adapted for specific tasks or aligned with human values. **Fine-tuning** is the process of adjusting a pretrained model to perform well on a particular task or within a specific domain.
