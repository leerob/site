...
export const getPostBySlug = async ({ slug }: { slug: string }): Promise<TPost> => {
  const preview = true;
  const { post } = await getClient(preview).fetch(postBySlugQuery, { slug })
  return post
}
export const getUpdatedPostSlug = async ({id}: {id: string}): Promise<string> => {
...
export const getSnippet = async ({ slug }: { slug: string }): Promise<TSnippet> => {
