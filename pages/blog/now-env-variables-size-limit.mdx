...
export const meta = {
date: '2020-02-06',
description: `Learn how to encrypt secrets to workaround ZEIT Now's 4kb max limit for environment variables.`,
image: '/static/images/now-env-variables-size-limit/banner.png',
...
# How To Use Encrypted Secrets With Now

<DateAndViews date={meta.date} id={meta.slug} />

When trying to deploy a new API route with [Now](https://zeit.co), I stumbled onto this error.
...
I _love_ Now and use it for all my deployments. Thus, I had 12 different secrets. Unfortunately, there is an [upstream limit](https://zeit.co/docs/v2/platform/limits#environment-variables) on the size.
...
We can add secrets using the Now CLI.

```bash
$ now secret add google-encryption-iv
$ now secret add google-encryption-secret
```
