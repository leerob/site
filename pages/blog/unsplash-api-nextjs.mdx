import DateAndViews from '../../components/date-and-views'
import Post from '../../components/post'
import Link from '../../components/link'
import Unsplash from '../../components/metrics/unsplash';

export const meta = {
date: '2020-02-09',
description: 'TODO',
image: '/static/images/metrics.png',
slug: 'unsplash-api-nextjs',
title: 'Create a Dashboard with Next.js API Routes - Unsplash API'
}

export default ({children}) => <Post meta={meta}>{children}</Post>

# Create a Dashboard with Next.js API Routes - Unsplash API

<DateAndViews date={meta.date} id={meta.slug} />

<!-- > Part 1 of 3 in my series on Creating a Dashboard with Next.js. -->

> In this series, you will learn how to <Link slug="dashboard" title="Lee Robinson - Dashboard">create a dashboard</Link> using Next.js API routes and integrate with third-party APIs.

<Unsplash />

## Overview

[Unsplash](https://unsplash.com) is the largest source of high-quality images on the internet.
Amateur and professional photographers give back by allowing their photos to be used [100% free](https://unsplash.com/license) for commerical and non-commerical usage.

They also provide a [wonderful API](https://unsplash.com/developers), which gives you access to millions of photographs and a variety of metrics.
As an [Unsplash contributor](https://unsplash.com/@leerob), I wanted to see how many times my photos have been viewed and downloaded.

## Getting Access to Unsplash

To gain access to the Unsplash API, you will need to:

-   [Create an account.](https://unsplash.com/documentation#creating-a-developer-account)
-   Go to [your applications](https://unsplash.com/oauth/applications).
-   Click “New Application”.
-   Accept the terms of usage.
-   Fill in the required details.
-   Retrieve the secret key for your new application.

**That's it!** 🎉 You now have access to make requests to the Unsplash API with your secret key.

## Creating an environment variable

To securely access the API, we will need to include the secret with each request.
We also _do not_ want to commit secrets to git. Thus, we should use an environment variable.

Since I'm deploying via [Now](https://zeit.co), I can add the secret via their CLI.

```bash
$ now secret add unsplash-access-token
```

To reference it locally when using `now dev`, it needs to be added to your `.env` file.

```bash
UNSPLASH_ACCESS_TOKEN=042d344
```

Finally, we need to add our new secret to `now.json`.

```json
{
    "env": {
        "UNSPLASH_ACCESS_KEY": "@unsplash-access-key"
    }
}
```

We can now reference the secret via `process.env.UNSPLASH_ACCESS_KEY` in our API route.

## Creating the API Route

Let's start by creating a new API route at `pages/api/unsplash.js`.

```js
export default (req, res) => {
    return res.status(200).json({});
};
```

To make server-side requests to Unsplash, we can install the [unsplash-js](https://github.com/unsplash/unsplash-js) library.
Since the library [depends on fetch](https://github.com/unsplash/unsplash-js#dependencies), you need to include `isomorphic-unfetch` (or another fetching library like `node-fetch`).

```bash
$ yarn add unsplash-js isomorphic-unfetch
```

Then, we can initialize `unsplash-js` inside our API route.

```js
import fetch from 'isomorphic-unfetch';
import Unsplash, {toJson} from 'unsplash-js';

let unsplash;

export default async (_, res) => {
    if (!unsplash) {
        unsplash = new Unsplash({
            accessKey: process.env.UNSPLASH_ACCESS_KEY
        });
    }

    return res.status(200).json({});
};
```

Finally, we want to pull statistics for a given user and return the total number of downloads and views.

```js
import fetch from 'isomorphic-unfetch';
import Unsplash, {toJson} from 'unsplash-js';

let unsplash;

export default async (_, res) => {
    if (!unsplash) {
        unsplash = new Unsplash({
            accessKey: process.env.UNSPLASH_ACCESS_KEY
        });
    }

    const userStats = await unsplash.users.statistics('leerob');
    const {downloads, views} = await toJson(userStats);

    return res.status(200).json({
        downloads: downloads.total,
        views: views.total
    });
};
```

## Example

This isn't an image - it's pulling real data!

<Unsplash />

## Keep Reading

**Create a Dashboard with Next.js API Routes**

-   Part 1 - <Link slug="blog/unsplash-api-nextjs" title="Create a Dashboard with Next.js API Routes - Unsplash API">Unsplash API</Link>
-   Part 2 - YouTube API (Coming Soon!)
-   Part 3 - Google Analytics API (Coming Soon!)
-   Part 4 - Fetching Data with SWR (Coming Soon!)
