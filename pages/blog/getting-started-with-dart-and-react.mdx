...
[dart_dev](https://github.com/Workiva/dart_dev) is our centralized tooling built on top of the Dart SDK. All Dart projects eventually share a common set of development requirements:
...
[OverReact](https://workiva.github.io/over_react/) is our library for building statically-typed React UI components. Since OverReact is built on top of React JS, I strongly encourage you to gain familiarity with React first if you're not by reading [this tutorial](https://reactjs.org/docs/hello-world.html). The example below compares a `render()` function for JSX and OverReact that will have the exact same HTML markup result.
...
[w_flux](https://github.com/Workiva/w_flux) is our architecture library for Flux - a simple uni-directional data flow pattern that provides an MVC like architecture and works well with React UI components.
