---
title: 'Which Backend Should I Use As A Front-End Developer?'
...
and then present a few options. Before we can begin our journey, I must first explain the basics.

## Table of Contents

- [Backend Basics](#backend-basics)
- [Do You Even Need a Database?](#do-you-even-need-a-database)
- [Low Code Solutions](#low-code-solutions)
- [Terms To Understand](#terms-to-understand)
  - [SQL / NoSQL](#sql-nosql)
  - [GraphQL / REST](#graphql-rest)
  - [Authentication / Authorization](#authentication-authorization)
  - [Hosted / Self-Hosted](#hosted-self-hosted)
...
- [List of Services](#list-of-services)
  ...
  - [Prisma](#prisma)
  - [Others](#others)
- [Should You Use a CMS?](#should-you-use-a-cms)
...
- You need a place to store data ([Database](https://en.wikipedia.org/wiki/Database))
...
You don't have to consider what happens in between. On a long enough timeline, you'll eventually need to build your own API or
work with an existing codebase that has a back-end. Which brings us to our first question...

## Do You Even Need a Database?

You need a place to store data, right? Here's some questions to consider:
...
you probably want to use [Markdown](https://guides.github.com/features/mastering-markdown/)
with [Git](https://git-scm.com/). That's exactly what [this blog](https://github.com/leerob/leerob.io) uses!
It's also worth mentioning you _could_ use JSON files as a psuedo-database if you fall into this category.

However, you're likely reading this article because **you need a database**. Before we start learning about how you can create
you own backend, you might also consider a no-code solution.

## Low Code Solutions

Since you're a front-end developer, you might be able to utilize a low code solution.
This is arguably the fastest way to get access to some remote data source.
...
as a database. There are other services that provider an [API abstraction](https://nocodeapi.com/) on a variety of data sources.
...
First, I need to explain some terms you'll likely to run into.

### SQL / NoSQL

A SQL database contains [relational data](https://aws.amazon.com/relational-database/). Consider a social media site with a table for `users` and `posts`.
...
This makes it easy to look up different pieces of related data through queries.
...
If you want to learn more, [here's a good resource](https://hasura.io/learn/graphql/intro-graphql/graphql-vs-rest/).

### Authentication / Authorization

Authentication is the process of verifying who a user is, while authorization is the process of verifying what they have access to.
You'll commonly see this abbrievated as AuthN (Authentication) and AuthZ (Authorization).
...
When learning about database relationships, you might hear terms like [one-to-many](<https://en.wikipedia.org/wiki/One-to-many_(data_model)>)
and [many-to-many](<https://en.wikipedia.org/wiki/Many-to-many_(data_model)>). These are ways of modeling your data.

## Levels of Abstraction

The reason this blog post will ask a lot of questions is because every situation is unique.
There are _many_ tools that exist to solve the same problem. By understanding what you're looking for,
...
Personally, I like to experiement with new technologies when I'm creating personal projects. If I'm
...
tech stack yourself. As you introduce other engineers into the mix, things get more difficult.
...
Let's start with assuming you want something more high-level, ideally with a website or interface to manage your data.
If you prefer a more bare metal approach, you might consider [starting from scratch](#starting-from-scratch).
...
The reality is that each service listed here has it's own tradeoffs. Hopefully this gives you a high-level overview to provide
an entry point to research more. But first, some questions to consider.
...
4. Reliability (ACID, backups, etc)
5. Subscriptions / realtime
6. Tooling (migrations, etc) -->

| Name                                                         | Type      | Open Source | Free Tier | Auth Included | GraphQL |
| ------------------------------------------------------------ | --------- | ----------- | --------- | ------------- | ------- |
| [Firebase](https://firebase.google.com/)                     | NoSQL     | ⛔️         | ✅        | ✅            | ⛔️     |
| [MongoDB](https://www.mongodb.com/)                          | NoSQL     | ✅          | ✅        | ✅            | ⛔️     |
| [AWS DynamoDB](https://aws.amazon.com/dynamodb/)             | NoSQL     | ✅          | ⛔️       | ⛔️           | ⛔️     |
| [Fauna](https://fauna.com/)                                  | NoSQL     | ⛔️         | ✅        | ✅            | ✅      |
| [Hasura](https://hasura.io/)                                 | Postgres  | ✅          | ✅        | ⛔️           | ✅      |
| [Nhost](https://nhost.io/)                                   | Postgres  | ✅          | ✅        | ✅            | ✅      |
| [Supabase](https://supabase.io/)                             | Postgres  | ✅          | ✅        | ✅            | ⛔️     |
| [Prisma](https://prisma.io)                                  | SQL-based | ✅          | ✅        | ⛔️           | ⛔️     |
| [Prisma](https://prisma.io) + [Nexus](https://nexus.js.org/) | SQL-based | ✅          | ✅        | ⛔️           | ✅      |
| [AWS RDS](https://aws.amazon.com/rds/)                       | SQL-based | ✅          | ⛔️       | ⛔️           | ⛔️     |
| [AWS Amplify](https://aws.amazon.com/amplify/)               | Both      | ✅          | ⛔️       | ✅            | ✅      |
...
<ProsCard
  ...
  pros={[
    ...
    'You want an estalished community that has many examples'
  ]}
/>
...
<ConsCard
  ...
  cons={[
    ...
    'You have very relational data. Some pain can be mitigated with composite indexes',
...
Hasura allows you to create an instant GraphQL API based on Postgres. With [Hasura Cloud](https://hasura.io/cloud/), it's extremely fast
to go from an idea to a live API. I love that you can create your database models directly through their console.
It's worth noting that Hasura is open-source, so you can run it self-hosted if you want.
...
<ConsCard
  ...
  cons={[
    ...
    'You want auth included in the platform'
  ]}
/>
...
If you want GraphQL, you can pair Prisma with [Nexus](https://nexus.js.org/).
...
I haven't worked very closely with the others listed, but here's some general information you might want to know.

- MongoDB can support GraphQL with [Stitch](https://developer.mongodb.com/how-to/graphql-support-atlas-stitch)
- If you're a founder or startup who hasn't raised any funding, you can apply for
  up to [\$1,000 in AWS RDS credits](https://aws.amazon.com/about-aws/whats-new/2020/02/aws-launches-activate-founders-package/).
- AWS is the largest player in the ecosystem.
- AWS Dynamo's pay-as-you-go pricing will be pennies a month up to 1 million requests per month scale.
- AWS RDS is [boring, proven technology](http://boringtechnology.club/).
...
If you answered yes to any of those question, you might want to explore a CMS. Here are [10 great options](https://bejamas.io/blog/headless-cms/).

## Starting From Scratch

If you read through all the questions above and thought "I want complete control", then you're probably in the wrong place.
...
Does anything look wrong? Is there anything you would change? Leave a comment below or send a pull request on GitHub.
