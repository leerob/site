...
```bash
$ now secret add MAILCHIMP_LIST_ID <secret-value>
$ now secret add MAILCHIMP_API_KEY <secret-value>
```
...
These variables will be available at build time. To test locally, we'll also want to create a `.env` file since Vercel Secrets [aren't available](https://zeit.co/docs/v2/serverless-functions/env-and-secrets/) locally running `now dev`.
