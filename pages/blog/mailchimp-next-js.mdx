...
After creating an account, you'll need to fetch your [API key](https://mailchimp.com/help/about-api-keys/#find+or+generate+your+api+key). You can utilize their [API Playground](https://us1.api.mailchimp.com/playground/?_ga=2.231663199.1180055433.1567349670-195619933.1565803046) to test requests.

When a user clicks "subscribe", we'll want to add their email address to our list. This is through the [List Members POST](https://developer.mailchimp.com/documentation/mailchimp/reference/lists/members/#create-post_lists_list_id_members) endpoint.
...
Where `dc` is the datacenter. This is the last 3 characters of our API key we retrieved earlier. We'll also need to fetch the [Audience ID](https://mailchimp.com/help/find-audience-id/) (or List ID) for our mailing list.

## Environment Variables

Let's make our secrets available without hardcoding them into our request. Since I'm deploying with [Now](https://zeit.co), I'll need to set up some [environment variables](https://zeit.co/docs/v2/build-step#using-environment-variables-and-secrets).
...
These variables will now be available at build time. To test locally, we'll also want to create an `.env` file since Now Secrets [aren't available](https://zeit.co/docs/v2/serverless-functions/env-and-secrets/) locally running `now dev`.
...
**Note:** Don't forgot to add `.env` to your `.gitignore`. We don't want to commit our secrets.

## Creating The Request

Now that we have the API Key and the List ID available, we can create an HTTP request to add a member to our list.
