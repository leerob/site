...
export const meta = {
date: '2019-02-12',
description: 'At Hy-Vee, we reached a point where we were spinning up new teams extremely quickly and needed to maintain consistency across products.',
...
![](/static/images/ui-component-library-with-styled-components/banner.jpg)
...
At <Link slug="blog/joining-hy-vee" title="Joining Hy-Vee" >Hy-Vee</Link>, we reached a point where we were spinning up new teams extremely quickly and needed to maintain consistency across products. How could we ensure every person created a button that looked and functioned the same way across all digital properties?
...
1. _Create consistency across the organization_

    - All digital properties should look and feel the same, implementing our agreed upon style guide.

2. _Improve the overall quality of our codebase_
...
3. _Increase developer proficiency_
...
```jsx
<Input disabled={false} id="first-name" label="First Name" placeholder="Please enter your name" required />
```
...
![](/static/images/ui-component-library-with-styled-components/storybook.gif)
...
This has inspired us to implement a <Link slug="blog/monorepo-lerna-yarn-workspaces" title="Creating a Monorepo with Lerna & Yarn Workspaces">Monorepo</Link> design where we can easily develop and publish a large number of packages.

Want to learn more? <Link slug="blog/monorepo-lerna-yarn-workspaces" title="Creating a Monorepo with Lerna & Yarn Workspaces">Read the next post</Link> where I'll create a Monorepo with [Lerna](https://lernajs.io/) and [Yarn Workspaces](https://yarnpkg.com/lang/en/docs/workspaces/).
