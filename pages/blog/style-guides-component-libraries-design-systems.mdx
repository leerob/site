...
For most smaller companies, I would argue that a Monorepo is unnecessary. It has its time and place. <Link slug="blog/monorepo-lerna-yarn-workspaces" title="Creating a Monorepo with Lerna & Yarn Workspaces">We use it at Hy-Vee</Link> (~150 developers) to distribute 10 different packages at once, spanning both internal and external apps.

**Note:** Want to use a Monorepo? Check out <Link slug="blog/monorepo-lerna-yarn-workspaces" title="Creating a Monorepo with Lerna & Yarn Workspaces">Creating a Monorepo with Lerna & Yarn Workspaces</Link>.
