...
![Next.js and Prisma](/static/images/next-prisma/prisma.gif)
...
```bash
$ git clone https://github.com/leerob/next-prism-static-site.git
$ cd next-prism-static-site
$ yarn
```
...
Prisma Migrate is still experimental, but it will be ready for production soon.
In the meantime, you can still perform schema migrations using plain SQL
or another migration tool if you choose.
...
This creates a new folder `prisma/migrations` tracking every change you've made.
...
![Prisma Studio](/static/images/next-prisma/studio.gif)

For convenience, I've added these three commands as scripts in the `package.json` already.
...
```js{4,5,6,7,8,9,10,20,21}:pages/songs/[id].js
import { PrismaClient } from '@prisma/client';
...
Deploying static sites couldn't be easier with [ZEIT Now](https://zeit.co).
After installing the Now CLI, simply run `now` in the root of the project.
...
and static site benefits result in an incredible user experience.

To see the completed tutorial code with styling added,
check out my [next-prisma](https://github.com/leerob/next-prisma) repo.
