...
-   I ported over my personal site from <Link slug="blog/migrating-to-hugo" title="Migrating to Hugo" underline={true}>Hugo</Link> to [Next.js + MDX](https://github.com/leerob/leerob.io).
-   I created [dsmtech.io](https://dsmtech.io/) (and explored using <Link slug="blog/using-mapbox-with-next-js" title="Using Mapbox with Next.js (Instead of Google Maps)" underline={true}>Mapbox</Link>).
...
If you want learn more, check out [Building a UI Component Library with Styled Components](https://medium.com/hy-vee-engineering).
...
My <Link slug="blog/beginners-guide-to-the-programming-portfolio" title="Beginner’s Guide to the Programming Portfolio" underline={true}>portfolio</Link> has transformed quite a bit since I started in 2014. Originally, it was
just static HTML & CSS. Then, I <Link slug="blog/migrating-to-hugo" title="Migrating to Hugo" underline={true}>switched over to Hugo</Link> so I could write my posts in Markdown.
...
that the page loads instantly. As of [Next.js 8](https://nextjs.org/blog/next-8/), `prefetch` uses `<link rel="preload">` instead of a `<script>` tag. It also only starts prefetching after `onload` to allow the browser to manage resources.
...
**Bonus:** Next won't prefetch [if the user is on 2G](https://github.com/zeit/next.js/blob/fc19b233ebcb6074a55ef4a00771ef7b0a3bcc33/packages/next/client/page-loader.js#L137-L143). Pretty neat!
