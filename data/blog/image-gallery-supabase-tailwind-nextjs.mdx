...
summary: 'Learn how to create an image gallery with dynamic content from a PostgreSQL database, with support for content updates without neding to redeploy.'
...
Next, we can update our placeholder for images to use the data fetched from Supabase. We'll also use our new `BlurImage` component, which makes use of `next/image` under the hood:
...
- [View the complete code](https://github.com/leerob/image-gallery-supabase-tailwind-nextjs)
- [Try the demo](https://swag.vercel.app)
