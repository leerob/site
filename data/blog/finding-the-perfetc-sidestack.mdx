...
for a stack that allows focusing on developing an idea that I have, leaving as much as possible all the annoyances behind.
...
and I am starting to work on a new project for an idea I have been playing with for a while. However, this time I wanted to take some time and invest a bit into a research on what tech stack to go with for the next years building this kind of projects.
...
it's going out really great and because it took some time to find the right pieces I though would be useful to share them with you.
...
solve real world problems with software. That is after all what I really like doing.
...
I was (and still partially am) excited about [Hasura](https://hasura.io/). I am a big fan of [GraphQL](https://graphql.org/) and that is something I was purposely chasing since it would allow me to keep the stack fairly close to my normal working setup. However [Hasura](https://hasura.io/) does not have authentication,
and the docs sounded like a bit more than I wanted to invest on (simplicity). I must admit I might give Hasura a try some day if I do regret stepping away from GraphQL.
...
As a bonus I recently found (and this helped a little validate my supabase choice) that [Vercel](https://vercel.com/) has a [starterkit](https://github.com/vercel/nextjs-subscription-payments) for sass and subscription projects
with already built-in support for [supabase](https://supabase.io/), [stripe](https://stripe.com/) (which I did not included in my choices because I think it's not a deal breaker, but in the only project I am currently charging I am using stripe so its great that it's already there) and also [tailwind](https://tailwindcss.com/) which
...
components on my side projects, but it works also as a very nice source of inspiration for work.
