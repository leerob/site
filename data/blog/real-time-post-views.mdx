...
`fetcher` is a small wrapper around `fetch` returning `json`. `data` will contain the JSON response from our API route.
Let's create a `ViewCounter` component to use SWR. I'm also using a helpful package [comma-number](https://www.npmjs.com/package/comma-number) for formatting, but you don't need this.
