...
image: '/static/images/posts/style.jpg'
tags: [web, angular, javascript]
---

<Image
  alt={`Sky`}
  src={`/static/images/posts/style.jpg`}
  width={1200}
  height={630}
  priority
/>
...
const components = angular.module('app.components', [Calendar, Events]).name;
...
const common = angular.module('app.common', [Nav, Footer]).name;
...
Data is passed to him through bindings, it then uses the $onChanges lifecycle hook to create a new instance for the todo binding to avoid reflection on the parent and instead only pass the data through the event _onAddTodo_ avoiding any kind of automatic update (two way binding) in the parent.
