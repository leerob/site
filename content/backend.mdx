...
2. I'm focusing on JavaScript and TypeScript codebases
...
- **Connectionless:** When compute scales to zero, you can't open a direct connection to the database on every new connection. This isn't a new problem, formerly seen when reaching scale and having thousands of concurrent connections to your database, but has reemerged with the popularity of serverless compute. Connectionless solutions provide an **abstraction** for the developer to avoid manually managing database connections. This is often exposed through an SDK or HTTP API.
...
For example, consider databases like Postgres. New solutions like Neon and Supabase abstract connection management, providing you with a simple way to query and mutate data. In the case of Supabase, there's a client library that uses an HTTP API [built on PostgREST](https://supabase.com/docs/guides/api):
...
> This solution has a key advantage over some others, such as HTTP fetch-based drivers: you get a real, ordinary Postgres connection via a familiar, ordinary Postgres driver.
...
There's even solutions like PlanetScale which can handle up to [a million connections](https://planetscale.com/blog/one-million-connections), which effectively also allows you to never think about managing connections.
...
For example, you can of course run MySQL or Postgres on the major cloud providers like AWS. There's a long tail of niche data storage solutions, so some will definitely be missing I haven't heard of or used.

### Established serverless solutions

- **Firebase**
  - **[Firestore:](https://firebase.google.com/docs/firestore)** Over 10 years old now, Firebase is a well-adopted document database. One unique advantage Firebase has it built-in support for authentication, real time workloads, and cross-platform support for mobile.
- **MongoDB**
  - [**Atlas Serverless:**](https://www.mongodb.com/use-cases/serverless) MongoDB has an entire data platform, including search / analytics / etc. Their recent investment into serverless is very exciting, with Atlas Serverless now [generally available](https://www.mongodb.com/blog/post/embrace-benefits-serverless-development-atlas). With their [Data API](https://www.mongodb.com/docs/atlas/api/data-api/), they're now a fantastic pair for serverless / edge.
- **MySQL**
...
- **PostgreSQL**
  ...
  - **[CockroachDB](https://www.cockroachlabs.com/blog/announcing-cockroachdb-serverless/): TODO**
...
- **Pre-GA**
  - **[Convex](https://www.convex.dev/)**: Fantastic for real time workloads, but also a very simple, type-safe interface for querying/mutations data. You write (and think) in functions. Pairs well with React's mental model.
  ...
  - **[Grafbase](https://grafbase.com/)**: If you love GraphQL, Grafbase is worth exploring. Designed to integrate into branch-based workflows + really fast reads globally.
...
- **Generally Available (GA)**
  ...
  - **[Supabase](https://supabase.com/):** Open-source, built-on Postgres. Database + Auth + [Storage](https://supabase.com/storage) and more.
...
- **Cloud Provider Offerings:** AWS RDS, AWS Dynamo, Azure SQL, Azure CosmosDB, Google Cloud SQL, Google BigTable, and more. [AlloyDB](https://cloud.google.com/alloydb) from Google is particularly interesting.
...
_Thanks to [Guillermo Rauch](https://twitter.com/rauchg), etc, for reviewing this post._
