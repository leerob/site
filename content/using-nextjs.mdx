...
In Next.js 14, for example, if you want to opt-out of caching, you would use [`noStore()`](https://nextjs.org/docs/app/api-reference/functions/unstable_noStore) versus `cache: 'no-store'` on the `fetch`. And if you want to use more programatically use caching and revalidating features, those will soon be standalone APIs as well.
